...
export default function HomePage({ session, profile }) {
  ...
  const load = async () => {
    const [{ data: its }, { data: cats }] = await Promise.all([
      supabase.from('items')
        .select('*, category:category_id(id, label, emoji), interests(*, profiles(display_name, avatar_color))')
    ...
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '28px 16px', fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '28px', flexWrap: 'wrap' }}>
        {[
          ...
          { v: unwanted, l: 'Ingen vil ha', emoji: '😔', click: () => setFilterUnwanted(!filterUnwanted) },
        ].map(s => (
          <div key={s.l} onClick={s.click} style={{
            flex: 1, minWidth: '110px', background: '#fff',
            border: `1px solid ${s.warn && s.v > 0 ? '#e8c4a0' : '#e8e0d6'}`,
            borderRadius: '10px', padding: '16px 18px',
            cursor: s.click ? 'pointer' : 'default',
          }}>
            <div style={{ fontSize: '22px', marginBottom: '2px' }}>{s.emoji}</div>
            <div style={{ fontSize: '26px', color: s.warn && s.v > 0 ? '#c4855a' : '#2a1f14', fontFamily: 'Georgia, serif' }}>{s.v}</div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '24px', flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setFilterMine(!filterMine)} style={{
          ...
          background: filterMine ? '#2a1f14' : '#fff', color: filterMine ? '#f5f0eb' : '#2a1f14',
        ...
        <button onClick={() => navigate('/add')} style={{
          padding: '9px 20px', background: '#2a1f14', color: '#f5f0eb',
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '80px 0', color: '#a89080' }}>Laster…</div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '80px 0', color: '#a89080' }}>
          <div style={{ fontSize: '56px', marginBottom: '16px' }}>📦</div>
          <p style={{ fontSize: '16px', marginBottom: '20px' }}>Ingen gjenstander å vise.</p>
          <button onClick={() => navigate('/add')} style={{ padding: '12px 28px', background: '#2a1f14', color: '#f5f0eb', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '15px', fontFamily: 'DM Sans, sans-serif' }}>
            Legg til første gjenstand
          </button>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '16px' }}>
          {filtered.map(item => <ItemCard key={item.id} item={item} userId={session.user.id} onClick={() => navigate(`/item/${item.id}`)} />)}
        </div>
      )}
    </div>
  )
...
function ItemCard({ item, userId, onClick }) {
  const cat = item.category || { emoji: '📦', label: 'Annet' }
  ...
  return (
    ...
    onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = 'none' }}>
      <div style={{ height: '150px', background: '#f0ebe4', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative' }}>
        ...
        {hot && <span style={{ position: 'absolute', top: '8px', right: '8px', background: '#c4855a', color: '#fff', fontSize: '10px', padding: '2px 7px', borderRadius: '20px' }}>🔥 Ettertraktet</span>}
      </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: '3px' }}>
            {(item.interests || []).slice(0, 5).map(x => (
              <div key={x.id} title={x.profiles?.display_name} style={{ width: '24px', height: '24px', borderRadius: '50%', background: x.profiles?.avatar_color || '#ccc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', color: '#fff', fontWeight: '600' }}>
